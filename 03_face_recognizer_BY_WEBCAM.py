...
def recognize(img,
    ...
    for res in results:
        ...
        if name == 'unknown':
            ...
            cv2.putText(img, name, pt_1, cv2.FONT_HERSHEY_DUPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
        else:
            cv2.rectangle(img, pt_1, pt_2, (0, 255, 0), 2)
            cv2.putText(img, name + f' {proba:.2f}%', (pt_1[0], pt_1[1] - 5), cv2.FONT_HERSHEY_DUPLEX, 1,
...
if __name__ == '__main__':
    ...
    #vc = cv2.VideoCapture('data/video/<Nom_video>.mp4')
    while vc.isOpened():
        ret, frame = vc.read()
